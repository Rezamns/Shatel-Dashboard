...
$(document).ready(function() { 
  if(typeof news !== 'undefined' && news.length > 0) {
    var allNews =[];
      ...
      news.forEach(function(news){
        ...
        newsBox.find('.actions a').attr('href',news.url || '#');
  ...
  if(news.url)
    {
      newsBox.find('.action').append('<a href"' + news.url + '"><button class="button news-button" title="ادامه خبر">...ادامه خبر</button></a>')
    }